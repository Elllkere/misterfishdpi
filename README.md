...
Поддержка продукта прекращена, последний релиз [v25.1229.1430](https://github.com/Elllkere/misterfishdpi/releases/tag/v25.1229.1430). В связи с непопулярностью решения и неоправданными затратами, новые релизы решения выпускаться не будут. Новые коммиты могут появляться, но лишь для личного пользования, без релизной сборки. Используйте другие популярные решения
