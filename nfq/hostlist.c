...
static bool LoadIncludeIpListsForProfile(struct desync_profile* dp)
{
	if (!LoadHostLists(&dp->iplist, &dp->iplist_files))
		return false;
...
bool LoadIncludeIpLists()
{
	struct desync_profile_list* dpl;
	LIST_FOREACH(dpl, &params.desync_profiles, next)
		if (!LoadIncludeIpListsForProfile(&dpl->dp))
			return false;
	return true;
}
