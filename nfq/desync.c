...
static bool dp_match(
	struct desync_profile *dp, bool ipv6, uint16_t tcp_port, uint16_t udp_port, struct in_addr src, struct in_addr dst, const char *hostname,
	bool *bCheckDone, bool *bCheckResult, bool *bExcluded)
{
	...
	if (dp_match_l3l4(dp,ipv6,tcp_port,udp_port))
	{
		...
		{
			{
				const char* p = inet_ntoa(src);
				bool bInHostList = StrPoolCheckStr(dp->iplist, p);
				DLOG("Iplist check for src %s : %s\n", p, bInHostList ? "positive" : "negative");
				if (bInHostList) return true;
			}
			{
				const char* p = inet_ntoa(dst);
				bool bInHostList = StrPoolCheckStr(dp->iplist, p);
				DLOG("Iplist check for dst %s : %s\n", p, bInHostList ? "positive" : "negative");
				if (bInHostList) return true;
			}
...
static struct desync_profile *dp_find(
	struct desync_profile_list_head *head, bool ipv6, uint16_t tcp_port, uint16_t udp_port, struct in_addr src, struct in_addr dst, const char *hostname,
	bool *bCheckDone, bool *bCheckResult, bool *bExcluded)
{
	...
	{
		if (dp_match(&dpl->dp,ipv6,tcp_port,udp_port, src, dst, hostname,bCheckDone,bCheckResult,bExcluded))
		{
...
static uint8_t dpi_desync_tcp_packet_play(bool replay, size_t reasm_offset, uint32_t fwmark, const char *ifout, uint8_t *data_pkt, size_t *len_pkt, struct ip *ip, struct ip6_hdr *ip6hdr, struct tcphdr *tcphdr, size_t transport_len, uint8_t *data_payload, size_t len_payload)
{
	...
	if (replay)
	{
		...
		{
			dp = ctrack_replay->dp = dp_find(&params.desync_profiles, !!ip6hdr, ntohs(bReverse ? tcphdr->th_sport : tcphdr->th_dport), 0, ip->ip_src, ip->ip_dst, ctrack_replay->hostname, NULL, NULL, NULL);
	...
	else
	{
		...
		{
			dp = dp_find(&params.desync_profiles, !!ip6hdr, ntohs(bReverse ? tcphdr->th_sport : tcphdr->th_dport), 0, ip->ip_src, ip->ip_dst, ctrack ? ctrack->hostname : NULL, NULL, NULL, NULL);
			if (ctrack)
			{
				ctrack->dp = dp;
		...
		if (bHaveHost)
		{
			...
			if (ctrack_replay)
			{
				if (!ctrack_replay->hostname)
				{
					...
					dp = ctrack_replay->dp = dp_find(&params.desync_profiles, !!ip6hdr, ntohs(bReverse ? tcphdr->th_sport : tcphdr->th_dport), 0, ip->ip_src, ip->ip_dst, ctrack_replay->hostname, &ctrack_replay->bCheckDone, &ctrack_replay->bCheckResult, &ctrack_replay->bCheckExcluded);
					ctrack_replay->dp_search_complete = true;
					if (!dp)
					{
						reasm_orig_cancel(ctrack);
...
static uint8_t dpi_desync_udp_packet_play(bool replay, size_t reasm_offset, uint32_t fwmark, const char *ifout, uint8_t *data_pkt, size_t *len_pkt, struct ip *ip, struct ip6_hdr *ip6hdr, struct udphdr *udphdr, size_t transport_len, uint8_t *data_payload, size_t len_payload)
{
	...
	if (replay)
	{
		...
		{
			dp = ctrack_replay->dp = dp_find(&params.desync_profiles, !!ip6hdr, 0, ntohs(bReverse ? udphdr->uh_sport : udphdr->uh_dport), ip->ip_src, ip->ip_dst, ctrack_replay->hostname, NULL, NULL, NULL);
			ctrack_replay->dp_search_complete = true;
		}
		if (!dp)
		{
	...
	else
	{
		...
		{
			dp = dp_find(&params.desync_profiles, !!ip6hdr, 0, ntohs(bReverse ? udphdr->uh_sport : udphdr->uh_dport), ip->ip_src, ip->ip_dst, ctrack ? ctrack->hostname : NULL, NULL, NULL, NULL);
			if (ctrack)
			{
				ctrack->dp = dp;
				ctrack->dp_search_complete = true;
		...
		if (bHaveHost)
		{
			...
			if (ctrack_replay)
			{
				if (!ctrack_replay->hostname)
				{
					...
					DLOG("we have hostname now. searching desync profile again.\n");
					struct desync_profile *dp_prev = dp;
					dp = ctrack_replay->dp = dp_find(&params.desync_profiles, !!ip6hdr, 0, ntohs(bReverse ? udphdr->uh_sport : udphdr->uh_dport), ip->ip_src, ip->ip_dst, ctrack_replay->hostname, &ctrack_replay->bCheckDone, &ctrack_replay->bCheckResult, &ctrack_replay->bCheckExcluded);
					ctrack_replay->dp_search_complete = true;
