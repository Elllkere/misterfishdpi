...
static void dohup(void)
{
	if (bHup)
	{
		if (!LoadIncludeHostLists() || !LoadExcludeHostLists() || !LoadIncludeIpLists())
		{
...
int main(int argc, char **argv)
{
	...
	*pidfile = 0;
	...
	const struct option long_options[] = {
		...
		{"iplist",optional_argument,0,0},	// optidx=65
#endif
		{NULL,0,NULL,0}
	};
	if (argc < 2) exithelp();
	while ((v = getopt_long_only(argc, argv, "", long_options, &option_index)) != -1)
	{
		...
		case 64: /* nlm-list */
			...
			break;
		case 65: /* iplist */
			if (!strlist_add(&dp->iplist_files, optarg))
			{
				DLOG_ERR("strlist_add failed\n");
				exit_clean(1);
			}
	...
	if (!LoadIncludeIpLists())
	{
		DLOG_ERR("Include iplists load failed\n");
		exit_clean(1);
	}
