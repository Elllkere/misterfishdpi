...
void Zapret::getArgs(const std::string& id_name, std::string& args, const std::string& cur_path)
{
    ...
    {
        if (vars::provider < providers_list::PROVIDER_MTS)
        {
            ...
            if (vars::provider == providers_list::PROVIDER_OTHER)
            {
        ...
        else if (vars::provider == providers_list::PROVIDER_MTS)
        {
            args = std::format("--wf-tcp=80,443 --wf-udp=443 ");
        ...
        else if (vars::provider == providers_list::PROVIDER_LOVIT)
        {
            args = std::format("--wf-tcp=80,443 --wf-udp=443 ");
    ...
    else if (id_name == "discord")
    {
        ...
        {
            args += std::format("--filter-tcp=443 --hostlist=\"{}\\{}\" --dpi-desync=fake --dpi-desync-ttl=5 --new ", cur_path, discord);
