...
void Zapret::getArgs(const std::string& id_name, std::string& args, const std::string& cur_path, bool hostlist)
{
    ...
    if (id_name == "shared_youtube_service")
    {
        if (vars::provider < 2)
        {
            ...
            args += std::format("--filter-udp=443 --hostlist=\"{}\\{}\" --dpi-desync=fake --dpi-desync-repeats=11 --new ", cur_path, txt);
            ...
            args += std::format("--hostlist=\"{}\\{}\" --dpi-desync=fake,disorder2 --dpi-desync-autottl=2 --dpi-desync-fooling=md5sig --new ", cur_path, txt);

            if (vars::provider == 0)
                args += std::format("--filter-tcp=443 --hostlist=\"{}\\{}\" --dpi-desync=fake,split2 --dpi-desync-autottl=2 --dpi-desync-fooling=md5sig --dpi-desync-fake-tls=\"{}\\tls_clienthello_www_google_com.bin\"", cur_path, txt, cur_path);
            else
