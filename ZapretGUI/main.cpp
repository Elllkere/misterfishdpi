...
bool extractFile(const std::string& zipFilePath, const std::string& outputDir) 
{
    ...
    if (zipFile == nullptr) 
    {
        tools::sendNotif("Не удалось открыть архив", "", true);
        return false;
    }

    if (unzGoToFirstFile(zipFile) != UNZ_OK) 
    {
        tools::sendNotif("Не удалось найти первый файл", "", true);
        unzClose(zipFile);
        return false;
    }
    ...
    do {
        ...
        {
            tools::sendNotif("Не удалось получить файл", "", true);
        ...
        {
            tools::sendNotif("Не удалось открыть файл", "", true);
        ...
        if (bytesRead < 0) 
        {
            tools::sendNotif("Не удалось прочитать файл", "", true);
        ...
        else
        {
            ...
            {
                tools::sendNotif("Не удалось разархивировать файл", "", true);
                unzCloseCurrentFile(zipFile);
...
void update(const std::string& version, const LPSTR& lpCmdLine)
{
    ...
    {
        tools::sendNotif("Не удалось найти ссылку на новую версию", "", vars::notif != 0);
        return;
    }
    else if (res != S_OK)
    {
        tools::sendNotif(std::format("Не удалось скачать новую версию: {}", res), "", vars::notif != 0);
        return;
    }
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    if (strstr(lpCmdLine, "/waitupdate") || strstr(lpCmdLine, "-waitupdate"))
    {
        ...
        if (vars::bNotify_changes && vars::notif == 0)
        {
            ...
            if (result && !answer.empty() && !strstr(answer.c_str(), "error"))
            {
                ...
                tools::sendNotif(std::format("Обновление {}", vars::version), answer, false, false);
            }
        }
    }
    ...
    vars::services =
    {
        ...
        new SharedZapret(proton_width, proton_height, "Proton (без mail)", "proton", proton_texture, shared_service_youtube),
    ...
    if (vars::bStart_ver_check)
    {
        ...
        else if (answer != vars::version)
        {
            if (!vars::bAuto_update)
                tools::sendNotif("Вышла новая версия, скачать можно в настройках", "", vars::notif != 0);
#ifndef _DEBUG
            else
            {
                tools::sendNotif("Вышла новая версия", "Программа обновляется в фоне", false, false);
    ...
    else
    {
        ...
        tools::sendNotif("Не удалось проверить версию", "", vars::notif != 0);
    }
    ...
    while (!quit)
    {
        ...
        for (auto* service : vars::services)
        {
            ...
            if (!service->isRunning() && service->active)
            {
                if (g_winwsRetries == 5)
                {
                    tools::sendNotif("winws.exe был неожиданно закрыт. Перезапустить процесс не удалось", "", vars::notif != 0);
                    exit(0);
                }
        ...
        if(ImGui::BeginPopupModal("###note", NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove))
        {
            ...
            std::string text = "Это ваш первый запуск программы, это сообщение будет показано только 1 раз.\n";
            text += "Пожалуйста посетите раздел настройки и выберите нужного для себя провайдера,\n";
            text += "если вашего провайдера в списке нет и с выбранным 'другой' сервисы все равно не работают,\n";
            text += "то вы можете попробовать других провайдеров, возможно их настройки подойдут для вашего.\n";
            text += "Если с никаким провайдером не работают сервисы тогда можно создать issue на github";
        ...
        {
            std::vector<std::string> buttons = { "Сервисы", "Настройки", "SingBox"};
        ...
        ImGui::BeginChild("##right");
        {
            ...
            case 0:
            {
                ...
                for (int i = 0, showed = 0; i < vars::render_services.size(); i++)
                {
                    ...
                    if (ImGui::BeginPopupContextItem((std::string("##pop_") + service->id_name).c_str()))
                    {
                        ...
                        if (ImGui::MenuItem(hided ? "Показать" : "Скрыть"))
                        {
                        ...
                        if (vars::bHotkeys)
                        {
                            if (ImGui::MenuItem("Задать клавишу"))
                            {
                                open_key_bind = true;
                            }

                            ImGui::MenuItem(("Клавиша: " + std::string(tools::getKeyName(service->hotkey))).c_str(), 0, false, false);
                        }
                        ImGui::EndPopup();
                    }
                    ...
                    {
                        ImGui::Text("Нажмите любую клавишу...");
            ...
            case 1:
            {
                ...
                if (ImGui::Checkbox("Проверка версии при запуске", &vars::bStart_ver_check))
                {
                ...
                if (ImGui::Checkbox("Уведомлять об изменениях", &vars::bNotify_changes))
                {
                    if (!vars::bStart_ver_check)
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Будет работать только при типе уведомления - Windows уведомления");

                if (ImGui::Checkbox("Автоматическое обновление", &vars::bAuto_update))
                {
                    if (!vars::bStart_ver_check)
                ...
                if (ImGui::Checkbox("Автозапуск с windows", &vars::bWin_start))
                {
                ...
                if (ImGui::Checkbox("Автозапуск в свернутом состоянии", &vars::bTray_start))
                {
                    if (!vars::bWin_start)
                ...
                if (ImGui::Checkbox("Разблокировать Cloudflare", &vars::bUnlock_cf))
                {
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Cloudflare предоставляет собой сервис DDoS защиты.\nИз-за блокировки всего Cloudlfare много обычных не забаненных сайтов или игр не загружаются");

                if (ImGui::Checkbox("Разблокировать Akamai", &vars::bUnlock_akamai))
                {
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Akamai предоставляет собой сервис DDoS защиты (аналог cloudflare).\nИз-за блокировки Akamai некоторые не забаненые сайты или игры не загружаются");

                if (ImGui::Checkbox("Разблокировать Amazon", &vars::bUnlock_amazon))
                {
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Могут быть конфликты игр\\сайтов\nAmazon предоставляет множество серверов для сайтов, игр и прочего.\nИз-за блокировки Amazon некоторые не забаненые сайты или игры не загружаются");

                if (vars::bUnlock_amazon)
                {
                    ImGui::PushItemWidth(window::comboWidth);

                    if (ImGui::Combo("Тип разблокировки Amazon", &vars::amazon_type, tools::convertMapToCharArray(vars::amazon_types).data(), vars::amazon_types.size()))
                    {
                ...
                if (ImGui::Checkbox("Показать скрытые сервисы", &vars::bShow_hide))
                {
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Для установки клавиши нужно нажать ПКМ по сервису");

                ImGui::PushItemWidth(window::comboWidth);

                if (ImGui::Combo("Провайдер", &vars::provider, tools::convertMapToCharArray(vars::providers).data(), vars::providers.size()))
                {
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Представляет набор стратегий обычно работающих для выбранного провайдера.\nНе буквально провайдер. Можно выбирать любых");

                if (ImGui::Combo("Тип автозапуска", &vars::auto_start, tools::convertMapToCharArray(vars::auto_starts).data(), vars::auto_starts.size()))
                {
                    if (vars::bWin_start)
                    {
                        switch (vars::auto_start)
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("UAC - контроль учетных записей\n(окошко с подтверждением при запуске программы)\nПользовательский - запускает только когда вход выполнит конкретный пользователь");

                if (ImGui::Combo("Действие при Х", &vars::x_method, tools::convertMapToCharArray(vars::x_methods).data(), vars::x_methods.size()))
                {
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Что делать при нажатии на Х");

                if (ImGui::Combo("Тип уведомлений", &vars::notif, tools::convertMapToCharArray(vars::notifs).data(), vars::notifs.size()))
                {
                ...
                if (ImGui::SliderInt("Задержка автозапуска", &vars::start_delay, 1, 120))
                {
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Задержка перед запуском приложения при включенном автозапуске\n(для того чтобы интернет успел подключится)\nв секундах\nCTRL+клик \\ Tab чтобы активировать ввод");
                ...
                if (ImGui::Button("Завершить все обходы", ImVec2(200, 30)))
                {
                    tools::killAll();
                    tools::sendStop("sing-box.exe");
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Завершит все прошлые и нынешние процессы zapret");

                if (ImGui::Button("Проверить версию", ImVec2(200, 30)))
                {
                    std::string answer;
                    bool result = tools::request("https://elllkere.top/misterfish/version.txt", &answer, true);
                    if (!result || (answer.empty() || strstr(answer.c_str(), "error")))
                    ...
                    else
                    {
                        if (answer == vars::version)
                            tools::sendNotif("Версия актуальна", "", vars::notif != 0);
                        else
                        {
                            if (vars::bAuto_update)
                            {
                            ...
                            {
                                tools::sendNotif("Вышла новая версия", "", false, false);
            ...
            case 2:
            {
                ...
                bool proxy_ip = ImGui::InputTextWithHint("IP", "прокси ip", &vars::proxy_ip);
                bool proxy_port = ImGui::InputTextWithHint("порт", "прокси порт", &vars::proxy_port);
                bool proxy_user = ImGui::InputTextWithHint("пользователь", "необязательно", &vars::proxy_user);
                bool proxy_password = ImGui::InputTextWithHint("пароль", "необязательно", &vars::proxy_password, ImGuiInputTextFlags_Password);
                ...
                if (ImGui::Button("Перезагрузить", ImVec2(200, 30)))
                {
                    for (auto* singbox : vars::singbox_services)
                    {
                        if (singbox->active)
